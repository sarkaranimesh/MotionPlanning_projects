...
def plot_performance(time_history, ego_pos_history, ego_vel_history, lead_pos_history,
                    ...
                    desired_velocity, desired_distance, gap_history=None):
    """
    ...
    gs = fig.add_gridspec(6, 2)  # Added one more row for gap settings
    ...
    ax1.axhline(y=desired_velocity, color='r', linestyle='--', label='Desired Velocity')
    ...
    ax4.plot(time_history, ego_accel_history)
    ...
    ax5.plot(jerk_time, jerk)
    ax5.set_xlabel('Time (s)')
    ax5.set_ylabel('Jerk (m/s³)')
    ax5.grid(True)
    ax5.set_title('Jerk')
    ...
    ax6.hist(vel_error, bins=30, density=True)
    ax6.set_xlabel('Velocity Error (m/s)')
    ax6.set_ylabel('Density')
    ax6.grid(True)
    ax6.set_title('Velocity Error Distribution')
    ...
    ax7.hist(dist_error, bins=30, density=True)
    ax7.set_xlabel('Distance Error (m)')
    ax7.set_ylabel('Density')
    ax7.grid(True)
    ax7.set_title('Distance Error Distribution')
